...
class Tools extends CommonTools
{
    ...
    public static function listen($requestParamsList){
        ...
        if (isset($requestParamsList['echostr'])){
            $data = [];
            foreach ($requestParamsList as $key=>$val){
                $data[$key] = urldecode($val);
            }
            $signature = self::makeSignature($data);
            if ($signature != $data['msg_signature']){
                throw new \Exception('signature校验错误',-10023);
            }

            /*验证URL*/
            $string = '';
            $cryption = new CommonCryption($data['token'],$data['encoding_aes_key'],$data['appid']);
            $errCode = $cryption->verifyURL($data['msg_signature'],$data['timestamp'],$data['nonce'],$data['echostr'],$string);
            if ($errCode != 0) throw new \Exception('验证URL失败',-500);
        ...
        /*响应消息*/
        return self::response($requestParamsList);
    }
     ...
     * @throws \Exception
     */
    protected static function response($requestParamsList){
        /*判断是否设置公众号appid*/
        if (!isset($requestParamsList['appid'])){
            throw new \Exception('缺少必备的appid参数',-10004);
        }

        /*验证是否设置aes_key*/
        if (!isset($requestParamsList['encoding_aes_key'])){
            throw new \Exception('encoding_aes_key缺失',-10027);
        }

        /*检测消息签名*/
        if (!isset($requestParamsList['msg_signature'])){
            throw new \Exception('msg_signature缺失',-10028);
        }
    ...
    protected static function makeSignature($requestParamsList){
        /*生成签名*/
        $tmpArr = [$requestParamsList['token'],$requestParamsList['timestamp'],$requestParamsList['nonce'],$requestParamsList['echostr']];
